...
class Ventana_Tres_View(tk.Frame):
    ...
    def decorar_ventana(self):
        ...
        self.lblTitulo_servicio=tk.Label(self.parent,text="CHATBOT con OpenAI",font=('Roboto', 20))
        ...
        self.historial_de_conversacion.grid(row=1,column=0,padx=10,pady=0,ipadx=325,ipady=140)

        self.lblTexto_usuario=tk.Label(self.parent,text="Escribe acá",font=('Roboto', 10))
        self.lblTexto_usuario.grid(row=2,column=0)

        self.txtEntrada_texto_usuario = tk.Entry(self.parent, borderwidth=1, relief="solid")
        self.txtEntrada_texto_usuario.grid(row=3,column=0,padx=10,pady=0,ipadx=80,ipady=0)
    ...
    def botones_widget(self):      #Funcion que va dentro de configurar_ventana()
        self.btnEnviar_texto_hacia_IA = tk.Button(self.parent,text="Enviar")
        self.btnEnviar_texto_hacia_IA.grid(row=4, column=0)
